...
export function HealthSummarySection({ summary }: { summary?: HealthSummary }) {
  const stable = summary?.stable ?? true;
  const nextControlText = summary?.nextControlText ?? '—';
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <span
          className={`rounded-full px-3 py-1 text-xs font-semibold ${
            stable ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
          }`}
        >
          {stable ? 'Estable' : 'Atencion'}
        </span>
      </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-lg font-bold text-text-main">
              {stable ? 'Sin alertas inmediatas' : 'Revisa tus alertas'}
            </p>
