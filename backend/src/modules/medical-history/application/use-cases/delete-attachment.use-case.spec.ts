...
function makeSut() {
  ...
  const sut = new DeleteAttachmentUseCase(repo, accessService, storage);
  return { sut, repo, accessService, storage };
}

describe('DeleteAttachmentUseCase', () => {
  it('happy path: deletes attachment and S3 object, returns id', async () => {
    ...
    expect(repo.deleteAttachment).toHaveBeenCalledWith('attachment-1');
    expect(storage.deleteObject).toHaveBeenCalledWith('medical-history/patient-1/entry-1/uuid-report.pdf');
    expect(result).toEqual({ id: 'attachment-1' });
  });

  it('resilience: does not throw when S3 deleteObject fails', async () => {
    const { sut, repo, storage } = makeSut();
    repo.findEntryById.mockResolvedValueOnce(makeEntry());
    repo.findAttachmentById.mockResolvedValueOnce(makeAttachment());
