...
import { ListIncomingInvitationsUseCase } from './list-incoming-invitations.use-case';
...
function makeSut() {
  ...
  const sut = new ListIncomingInvitationsUseCase(queryService);
  return { sut, queryService };
}

describe('ListIncomingInvitationsUseCase', () => {
  it('happy path: returns incoming invitations for patient', async () => {
    const { sut, queryService } = makeSut();
    const actor = makeActor();

    const result = await sut.execute(actor);

    expect(queryService.listPendingIncomingForPatient).toHaveBeenCalledWith('patient-1');
