...
import { ActivateReminderUseCase } from './activate-reminder.use-case';
...
function makeSut() {
  ...
  const sut = new ActivateReminderUseCase(repo, access);
  return { sut, repo, access };
}

describe('ActivateReminderUseCase', () => {
  it('happy path: activates reminder and returns id', async () => {
